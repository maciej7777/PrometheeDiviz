...
NB_ERROR_TESTS=$(find tests -maxdepth 1 -type d -regex '.*/in_error[0-9]**\.v'"${version}"'$' | wc -l)
...
mkdir -p tests_tmp
for i in $(seq 1 ${NB_ERROR_TESTS}); do
    IN="tests/in_error${i}.v${version}"
    REFERENCE_OUT="tests/out_error${i}.v${version}"
    OUT=$(mktemp --tmpdir=. -d tests_tmp/out.XXX)
    echo "${IN}"
    ${CMD} "--v${version}" -i "${IN}" -o "${OUT}"
    diff -x README -ruBw "${REFERENCE_OUT}" "${OUT}"
